...
export default function WomenConsultant() {
    ...
    const submitOrder = async () => {
        ...
        try {
            ...
            if (data?.success) {
                ...
                try {
                    await supabase.functions.invoke('send-ebooks-email', {
                        body: {
                            userEmail: email,
                            userName: name,
                            phone: cleanPhone,
                            productName: 'womenconsult',
                            amount: finalAmount,
                            currency: 'IDR',
                            reference: data.reference || `WC-${Date.now()}`
                        }
                    });
                } catch (emailErr) { console.error('send-ebooks-email error', emailErr); }
            } else {
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#0D0A14', color: '#F5EFE0', minHeight: '100vh', overflowX: 'hidden' }}>
            ...
            <WhatsAppButton message="Kak mau nanya Women Consultant eL vision seperti apa caranya dan apa yang udah berhasil ?" />
        </div>
    );
}
