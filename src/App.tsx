...
const DarkFeminineTSX = () => {
    ...
    const [surveyCustomProblem, setSurveyCustomProblem] = useState('');

    const presenceQuestions = [
        "Saya merasa selalu diabaikan dan tidak dianggap penting",
        "Saya selalu mengejar pria dan ingin mengubah dinamika itu",
        "Saya kehilangan rasa percaya diri dan aura tarik saya",
        "Saya ingin menjadi wanita yang punya 'presence' dan bernilai tinggi",
        "Mau isi Sendiri"
    ];

    const perhatianQuestions = [
        "Pasangan saya mulai cuek, dingin, dan jarang membalas pesan",
        "Dia lebih mementingkan teman-temannya / hobinya daripada saya",
        "Hubungan mulai terasa hambar dan saya takut dia selingkuh",
        "Saya merasa selalu berkorban lebih banyak dari dia",
        "Mau isi Sendiri"
    ];
    ...
    const handleSupportSubmit = async () => {
        ...
        setSupportError('');
        try {
            const { data: paidEntry } = await (supabase as any)
                .from('global_product')
                .select('status')
    ...
    return (
      ...
      `}</style>
                    ...
                    {/* FLOATING WHATSAPP BUTTON — triggers support modal for paid users */}
                    <button
                    ...
                    {isSupportModalOpen && (
                        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                            <div style={{ background: '#111', border: '1px solid #333', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '400px', position: 'relative' }}>
                                <button 
                                    onClick={() => setIsSupportModalOpen(false)}
                                    style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: '#999', cursor: 'pointer' }}
                                >
                                    <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none"><path d="M18 6L6 18M6 6l12 12"></path></svg>
                                </button>
                                
                                <h3 style={{ margin: '0 0 12px 0', fontSize: '20px', color: 'var(--cream)' }}>Hubungi Admin</h3>
                                <p style={{ color: '#aaa', fontSize: '14px', marginBottom: '20px', lineHeight: '1.5' }}>
                                    Ada Pertanyaan atau kehilangan kontak admin? Silahkan Chat langsung.<br/><br/>
                                    Untuk Customer yang sudah membayar, silahkan masukan email Anda, dan Anda akan langsung diarahkan ke tim CS.
                                </p>
                                ...
                                <input 
                                    ...
                                    style={{ width: '100%', padding: '14px', borderRadius: '8px', border: '1px solid #333', background: '#000', color: 'white', marginBottom: '16px', fontSize: '15px' }}
                                />
                                
                                {supportError && (
                                    <div style={{ marginBottom: '16px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', padding: '12px', borderRadius: '8px' }}>
                                        <div style={{ color: '#ef4444', fontSize: '13px', lineHeight: '1.5' }}>{supportError}</div>
                                        <button onClick={() => window.location.href = '?pay'} style={{ marginTop: '10px', background: 'linear-gradient(135deg, var(--gold-dark), var(--gold-light))', color: '#000', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>
                                            Pesan Sekarang
                                        </button>
                                    </div>
                                )}
                                ...
                                <button 
                                    ...
                                    style={{ width: '100%', padding: '14px', borderRadius: '8px', background: '#25D366', color: '#fff', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: supportLoading ? 'not-allowed' : 'pointer', opacity: supportLoading ? 0.7 : 1 }}
                                >
                                    {supportLoading ? 'Memeriksa...' : 'Chat Admin'}
                                </button>
                            </div>
                        </div>
                    )}
                            ...
                            <div style={{ background: '#111', border: '1px solid #333', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '400px', position: 'relative' }}>
                                <button 
                                    onClick={() => {
                                        ...
                                        setSurveyStatus('');
                                        setSurveyProblem('');
                                        setSurveyCustomProblem('');
                                    }} 
                                    style={{ position: 'absolute', top: '12px', right: '12px', background: 'transparent', border: 'none', color: '#999', fontSize: '24px', cursor: 'pointer' }}
                                >
                                    &times;
                                </button>
                                <h3 style={{ margin: '0 0 16px 0', color: '#fff', fontSize: '18px', textAlign: 'center' }}>Konsultasi Spesial CS</h3>
                                <p style={{ color: '#ccc', fontSize: '14px', textAlign: 'center', marginBottom: '20px' }}>
                                    Agar kami bisa memberi jawaban memuaskan, silakan isi data berikut:
                                </p>

                                <div style={{ marginBottom: '16px' }}>
                                    <label style={{ display: 'block', color: '#fff', fontSize: '14px', marginBottom: '8px', fontWeight: 'bold' }}>Status Kamu:</label>
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        <button 
                                            onClick={() => setSurveyStatus('Single')}
                                            style={{ flex: 1, padding: '10px', borderRadius: '8px', border: surveyStatus === 'Single' ? '1px solid var(--gold-light)' : '1px solid #444', background: surveyStatus === 'Single' ? 'rgba(212,175,55,0.1)' : '#222', color: '#fff', cursor: 'pointer' }}
                                        >
                                            Single
                                        </button>
                                        <button 
                                            onClick={() => setSurveyStatus('Memiliki Pasangan')}
                                            style={{ flex: 1, padding: '10px', borderRadius: '8px', border: surveyStatus === 'Memiliki Pasangan' ? '1px solid var(--gold-light)' : '1px solid #444', background: surveyStatus === 'Memiliki Pasangan' ? 'rgba(212,175,55,0.1)' : '#222', color: '#fff', cursor: 'pointer' }}
                                        >
                                            Punya Pasangan
                                        </button>
                                    </div>
                                ...
                                {surveyStatus && (
                                    <div style={{ marginBottom: '16px' }}>
                                        <label style={{ display: 'block', color: '#fff', fontSize: '14px', marginBottom: '8px', fontWeight: 'bold' }}>Masalah yang ingin ditanyakan:</label>
                                        <select 
                                            value={surveyProblem}
                                            onChange={(e) => {
                                                setSurveyProblem(e.target.value);
                                                if (e.target.value !== 'Mau isi Sendiri') setSurveyCustomProblem('');
                                            }}
                                            style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #444', background: '#222', color: '#fff', fontSize: '14px', cursor: 'pointer' }}
                                        >
                                            <option value="">-- Pilih Masalahmu --</option>
                                            {(hasPerhatian ? perhatianQuestions : presenceQuestions).map((q, idx) => (
                                ...
                                {surveyProblem === 'Mau isi Sendiri' && (
                                    <div style={{ marginBottom: '20px' }}>
                                        <textarea 
                                            placeholder="Tuliskan masalahmu disini..."
                                            value={surveyCustomProblem}
                                            onChange={(e) => setSurveyCustomProblem(e.target.value)}
                                            style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #444', background: '#222', color: '#fff', fontSize: '14px', minHeight: '80px', resize: 'vertical' }}
                                        />
                                    </div>
                                )}

                                <button 
                                    disabled={!surveyStatus || !surveyProblem || (surveyProblem === 'Mau isi Sendiri' && !surveyCustomProblem.trim())}
                                    onClick={() => {
                                        const finalProblem = surveyProblem === 'Mau isi Sendiri' ? surveyCustomProblem : surveyProblem;
                                        const finalMessage = `Kak Saya ${surveyStatus}, masalah saya ${finalProblem}, Apakah ebook darkfeminine cocok untuk saya ? bagaimana isi nya ?`;
                                        window.open(`https://wa.me/62895325633487?text=${encodeURIComponent(finalMessage)}`, '_blank');
                                        setIsSurveyOpen(false);
                                    }}
                                    style={{
                                        width: '100%',
                                        ...
                                        borderRadius: '30px',
                                        border: 'none',
                                        background: (!surveyStatus || !surveyProblem || (surveyProblem === 'Mau isi Sendiri' && !surveyCustomProblem.trim())) ? '#444' : '#25D366',
                                        color: (!surveyStatus || !surveyProblem || (surveyProblem === 'Mau isi Sendiri' && !surveyCustomProblem.trim())) ? '#888' : '#fff',
                                        fontWeight: 'bold',
                                        fontSize: '15px',
                                        cursor: (!surveyStatus || !surveyProblem || (surveyProblem === 'Mau isi Sendiri' && !surveyCustomProblem.trim())) ? 'not-allowed' : 'pointer',
                                        marginTop: '10px'
                                    }}
                                >
                                    Lanjut ke WhatsApp
